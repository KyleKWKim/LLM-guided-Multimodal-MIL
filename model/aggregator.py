    ...
    def forward(self, x_list, x_CI):
        ...
        if ('CT' in self.args.modality) and ('pathology' in self.args.modality):
            ...
            if self.args.model_CT == 'resnetMC3_18':
                ...
                x_CT2CI, x_CI2CT = self.TwoWayTransformer_Both(x_input_CT, self.pe[:,:c].cuda(), self.fc_CI2CT(x_CI_prompted))
            elif self.args.model_CT == 'medicalNet':
                ...
                x_CT2CI, x_CI2CT = self.TwoWayTransformer_Both(x_input_CT, self.pe[:,:c*h*w].cuda(), self.fc_CI2CT(x_CI_prompted))
            
            # x_Pth2CI, x_CI2Pth = self.TwoWayTransformer_Pth(x_input_pathology, self.pe[:,:x_input_pathology.shape[1]].cuda(), x_CI_prompted)
